...
def ParseReviews(url):
	...
	tmp = data['recipe']['ingredients']
	for ingredient in ingredients:
		data['recipe']['ingredients'].append( {'ingridient': { 'quantity': ingredient[0], 'name': ingredient[1]}})
	#data['recipe']['ingredients'] = tmp
...
def ReadUrls():
	...
	g = open('data.xml','w')
	xml = dicttoxml(extracted_data)
	dom = parseString(xml)
	g.write(dom.toprettyxml())
	g.close()
